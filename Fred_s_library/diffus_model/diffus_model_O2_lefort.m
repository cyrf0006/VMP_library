function diffus_model_O2_lefort(O2_file, time0, diffus_value, diffus_method, hflx, dt, DT, L, topcondition)
...
% here modif for LEFORT <------------------------------
load bathy_Lefort
%time_topo = interp_time(1):(interp_time(end)-interp_time(1))./(length(interp_time))./length(x):interp_time(end);
dtt = length(interp_time)/length(x);
time_topo(1) = interp_time(1);
time_topo(length(x)) = interp_time(end);
for i = 2:length(x)-1
    time_topo(i) = interp_time(i*round(dtt));
end
H = H*0+300;
P_benthic = interp1(time_topo, flipud(H), interp_time);
% P_benthic is the depth where O2 is removed (benthic flux, atm_heatflx)
% end of modif
...
for t = 1:L/dt %loop on time
    ...
    T_mk = [0; T(1:length(T)-1)]; % T_k-1 <------- another lefort modif
    T_pk = [T(2:length(T)); 0]; % T_k+1    
    [Y, I] = min(abs(flipud(P)-P_benthic(t)));
    T_mk(I) = atm_heatflx(t); % inject benthic flux at "bottom"
    keyboard
    K2 = [0; K]; % reset K2   
    K2(I) = 1;
    
    %    keyboard
% $$$     T_av = T + C.*(K1.*(T_pk - T) - K2.*([0; T(2:end)] - T_mk)); % LE bon
   T_av = T + C.*(K1.*(T_pk - T) - K2.*(T - T_mk)); % test

    T_av(end) = topcondition; % impose T at 1st b in                              
    T_av(1) = topcondition; % impose T at 1st b in
