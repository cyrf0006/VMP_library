...
for i = 1:length(month)
    ...
    if isempty(II) == 1 % if no data for this month
        continue
    end
...
% $$$     dlmwrite(Toutname, [P T_ave T_error],'delimiter',' ','precision',6)
% $$$     dlmwrite(Soutname, [P S_ave S_error],'delimiter',' ','precision',6)
    dlmwrite(Toutname, [P T_ave T_2p5 T_97p5],'delimiter',' ','precision',6)
    dlmwrite(Soutname, [P S_ave S_2p5 S_97p5],'delimiter',' ','precision',6)

end %for i


figure(1)
print('-deps2', 'monthly_climT.eps')
