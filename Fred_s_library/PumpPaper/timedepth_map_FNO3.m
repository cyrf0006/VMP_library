function timedepth_map_FNO3(eps_files, epsMin, epsMax, varargin)

% function timedepth_eps(no_profile, epsMin, epsMax, varargin)
% was previously isopyc_tidecycle.m
% 
% ex: timedepth_map('epsProfiles1001.list', 1e-9, 1e-5, 100, 1)
%
% Now run from anywhere:
% timedepth_map_FNO3('~/PhD/WINDEX/MissionTadoussac/epsProfiles1001.list', 1e-9, 1e-5, 150, 1)
...
NO3mat = nan(size(P_N2,1), no_profile);
SAmat = nan(size(P_N2,1), no_profile);
...
% $$$         NO3(I) = NaN;
        ...
        for i = 1:length(zvec)
            ...
            SAmat(i, profile) = nanmean(SA(I));
            %            NO3mat(i, profile) = nanmean(NO3(I));
        end
    ...
    % clean NO3 / SA field 
    % (linear itp in time to remove nans)
% $$$     for i = 1:size(NO3mat,1)
% $$$         raw_vec = NO3mat(i,:);
% $$$         I = find(~isnan(raw_vec)==1);
% $$$         if ~isempty(I)==1 & length(I)>1
% $$$             raw_vec = raw_vec(I);
% $$$             vec_itp = interp1(timevec(I), raw_vec, timevec(I(1):I(end)));
% $$$             NO3mat(i,I(1):I(end)) = vec_itp;
% $$$         end
% $$$     end
...
% NO3 calculation
SAmat = sort(SAmat,1);
NO3mat = 7.3.*SAmat - 227; %32-34.3 SA (no winter)
I = find(SAmat<32 | SAmat>34.3);
NO3mat(I) = NaN;
...
[dNO3dx, dNO3dz] = gradient(NO3mat);
dNO3dz = dNO3dz./zbin;

% $$$ dNO3dz = diff(NO3mat, 1, 1)./zbin;
...
figure(1)
clf
set(gcf,'PaperUnits','centimeters','PaperPosition',[10 10 27 24])
% *********************** Adjust_space.m ************************ %
...
ylab = ylabel(cb,'log_{10} (F_{NO_3} / (mmol m^{-2} d^{-1}))', 'FontSize', 15)
ylab_pos = get(ylab, 'pos')
ylab_pos(1) = ylab_pos(1)+2
...
% Map
load '/media/Seagate Backup Plus Drive/cyrf0006_2014-03-26/data/SHC/500m/CHS_500m_gulf.mat'
...
m_text(lon_max, lat_min, 'Stat. 25 ', 'vertical', 'bottom', 'horizontal','right', 'color', [0 0 0],'FontSize',12, 'FontWeight', 'bold')
...
m_text(lonVec(1), latVec(1), datestr(timevec(1),15), 'vertical', ...
       'bottom', 'horizontal','right', 'color', [1 0 0]*.9,'FontSize',12, 'FontWeight', 'bold')
m_text(lonVec(end), latVec(end), datestr(timevec(end),15), 'vertical', ...
       'bottom', 'horizontal','right', 'color', [1 0 0]*.9,'FontSize',12, 'FontWeight', 'bold')
m_text(lonVec(60), latVec(60), datestr(timevec(62),15), 'vertical', ...
       'top', 'horizontal','right', 'color', [1 0 0]*.9,'FontSize',12, 'FontWeight', 'bold')
...
contourf(timevec, zvec, NO3mat, 50, 'linestyle', 'none')
%pcolor(timevec, zvec, NO3mat)
%shading interp

hold on
contour(timevec, zvec, NO3mat, [2:2:20], 'color', [0 0 0])
%contour(timevec, zvec, NO3mat, [1 1], 'k', 'linewidth', 2) 
%contour(timevec, zvec, rho_mat, [1023:.2:1026.6], 'linestyle', '-', 'linewidth', 1, 'edgecolor', 'k')
x = [timeVecTopo(1) timeVecTopo  timeVecTopo(end) timeVecTopo(1)];
y = [zmax botVecTopo zmax zmax];
patch(x, y, [1 1 1]*.6)
...
datetick('x',15)
ylabel('Depth (m)', 'fontsize', 15)
xlabel(datestr(timevec(1), 1), 'Fontsize', 15)
xlim([t0 tf])
ylab = ylabel(cb,'C_{NO_3} (mmol m^{-3})', 'FontSize', 15)
ylab_pos = get(ylab, 'pos')
ylab_pos(1) = ylab_pos(1)+2
set(ylab, 'pos', ylab_pos)
caxis([5 20])
set(gca, 'fontsize', 15)
text(datenum(2009, 10, 01, 20, 15, 0), 145, 'b', 'fontweight', 'bold', 'fontsize', 16)

adjust_space



print('-dpng', '-r300', 'timedepth_withFlux.png');
set(gcf, 'renderer', 'painters')
print('-depsc2', 'timedepth_withFlux.eps');
