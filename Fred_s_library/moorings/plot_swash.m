% called in swash_figures.m

[valong, vcross]=rotate_vecd(E_tide, N_tide, theta);
xtik = ceil(time_tide(1)*24)/24:2/24:floor(time_tide(end)*24)/24;
t1 = time_tide(1);
t2 = time_tide(end);
%datestr(xtik)
FONTSIZE = 10;
...
hold on
plot([HT(i) HT(i)], [Thab(1) Thab(end)], '--k', 'linewidth', 3)
hold off

subplot('position', [.22 .54 .65 .18])
%I = find(time_vec >= t1 & time_vec <= t2);
imagesc(time_tide, hab, valong*100);
set(gca, 'ydir', 'normal')
set(gca, 'xgrid', 'on')
set(gca, 'xtick', xtik)
set(gca, 'xticklabel', []);
...
set(gca, 'fontsize',FONTSIZE)
hold on
plot([HT(i) HT(i)], [hab(1) hab(end)], '--k', 'linewidth', 3)
hold off

subplot('position', [.22 .32 .65 .18])
imagesc(time_tide, hab, vcross*100);
set(gca, 'ydir', 'normal')
set(gca, 'xgrid', 'on')
set(gca, 'xtick', xtik)
set(gca, 'xticklabel', []);
...
set(gca, 'fontsize',FONTSIZE)
hold on
plot([HT(i) HT(i)], [habS(1) habS(end)], '--k', 'linewidth', 3)
hold off
...
res_shear = nanmean(S_tide, 2);

subplot('position', [.07 .54 .12 .18])
plot(res_along*100, hab, 'k')
set(gca, 'ydir', 'normal')
