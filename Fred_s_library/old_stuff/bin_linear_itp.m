...
month = 4:11;
yearnum=1993:2009; %this could be edited
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% --- load data (generated with climatology.m) --- %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% $$$     end
    ...
    J=find(diff(n_year)==0);
    if ~isempty(J)==1 % not empty
             
        for j=J %average with following
            Tyear(:, j+1)=nanmean([Tyear(:,j) Tyear(:, j+1)],2);
            Syear(:, j+1)=nanmean([Syear(:,j) Syear(:, j+1)],2);
        end
        I(J)=[];
        Tyear = Tmat(:,I);
        Syear = Smat(:,I);
    ...
    %interpolation on profile to remove NaN
    for prof = 1:length(I)
        II = find(~isnan(Tyear(:,prof))==1);
        if length(Tyear(II,prof))>1
            Tyear(:,prof) = interp1(depth(II),Tyear(II,prof), depth);
        else
            tprofile_tick(prof)=[];
        end
        
        II = find(~isnan(Syear(:,prof))==1);
        if  length(Syear(II,prof))>1
            Syear(:,prof) = interp1(depth(II),Syear(II,prof),depth);
        else
            sprofile_tick(prof)=[];
        end
    ...
    if i==length(yearnum);
        ...
        c = colorbar('FontSize', 10, 'position', [Pos(1)+Pos(3)+cbar_offset ...
                            Pos(2)+cbar_offset cbar_width Pos(4)-2* ...
                            cbar_offset]);
        set(c, 'tickdir', 'out')
