...
default_plot_rawaccel     = false;   % flag to plot unspiked accelerometer data
...
if make_figures && plot_kinematics && exist('Incl_X','var') && exist('Incl_Y','var')
    ...
    if ~strcmpi(profile_dir,'glide') % VMP - plot only theta X
        h = plot(X_data(:,1), P_slow + ADIS_P_delay,'linewidth',2); grid on
        xlabel('\theta_X [  ^{\circ} ]')
    else
        h = plot(X_data, P_slow + ADIS_P_delay,'linewidth',2); grid on
        hold all
        xlabel('[  ^{\circ} ]')
    ...
    sp(2) = subplot(1,MM,2);
    h = plot(dP_dt_LP, P_slow, 'linewidth', 2 ); grid on
    ...
    ll = {};
    if MM == 3 && strcmpi(profile_dir,'glide')
        sp(3) = subplot(1,MM,3);
        h = plot(speed_slow, P_slow ); grid on
        set(h(1), 'linewidth', 2)
        set(gca,'ylim',P_limits)
        set(gca, 'ydir', 'rev')
    ...
    if MM == 3 && exist('U_EM','var') 
        hold all
        h(2) = plot(U_EM,P_slow,'linewidth',1);
        ll{2} =texstr('U_EM');
        uistack(h(1),'top');
    end
    if MM == 3; legend(h,ll); end
    
    %-- show difference between fall rate and speed (if const speed is sed)
    if ~isempty(constant_speed) && (strcmp(profile_dir,'up') || strcmp(profile_dir,'down'))
        sp(3) = subplot(1,MM,3);
        h = plot(constant_speed-dP_dt_LP, P_slow ); grid on
...
if exist('Pitch','var') && make_figures && plot_kinematics
    ...
    %-- subplot of fall rate
    sp(2) = subplot(1,2,2);
    h = plot(dP_dt_LP, P_slow ); grid on
    set(h(1), 'linewidth', 2,   'color','b')
    set(gca, 'ydir', 'rev')
    set(gca,'yticklabel',[])
    xlabel('d\itP\rm / d\itt \rm[ dBar s^{-1}]')
...
if ~isempty(AA_linear) && make_figures && plot_kinematics && ~isempty(AA_linear_is_fast)
    ...
    %-- appropriaite independent variable
    ...
    ylabel('\it P \rm [dBar]')
    xlabel('[  ^{\circ} ]')
    legend('\theta_Y', '\theta_X','location','southeast')
    title(title_string)
    ...
    sp(3) = subplot(1,3,3);
    h = plot(AA_linear, P_data,'linewidth',2); grid on
    set(gca,'ylim',P_limits)
    set(gca, 'ydir', 'rev')
    xlabel('[m s^{-1}]')
...
if ~isempty(AA_piezo) && make_figures && (plot_kinematics || plot_rawaccel)
    ...
    if MM == 2
        sp(1) = subplot(1,MM,1);
        plot([AA_raw filtfilt(b, a, AA_raw)], P_fast);grid on
        title([title_string ['No despiking']])
        if ~strcmp(vehicle, 'sea_glider')
            xlim([-500 500]) % Don't set for sea glider
        end
    end
    
    %-- subplot: depiked acceleration 
    sp(MM) = subplot(1,MM,MM);
    plot([AA filtfilt(b, a, AA)], P_fast,'linewidth',2);grid on
    
    title([title_string despike_title])
    
    if exist('P_limits', 'var')
        set(sp,'ylim',P_limits)
    end
    if ~strcmp(vehicle, 'sea_glider')
        xlim([-500 500]) % Don't set for sea glider
    end
...
if exist('V_Bat','var') && plot_battery && make_figures
    ...
    h = plot([V_Bat filtfilt(b,a,V_Bat)], P_slow);grid on
    set(h(2),'linewidth',3,'color','m')
    set(gca, 'ydir', 'rev')
    ylabel('\it P \rm [dBar]')
    xlabel('\itV_{Bat}\rm [volts]')
    title(title_string)
    legend('V_{Bat}','V_{Bat}-LP','location','eastoutside')
    set(gca,'ylim',P_limits)
    drawnow
end
...
if we_have_S
    if we_have_JAC_T 
        ...
        % compute salinity
        S_CTD = salinity_JAC (P_slow, T_CTD, C_CTD,...
...
if make_figures && plot_sensors 
    ...
    if windows > 0
        ...
        plot_data = [T_CTD C_CTD S_CTD potential_density];
        ...
        for index = 1:windows
            ...
            % - plot data
            plot(plot_data(:,index), P_slow + offset);grid on
...
if   make_figures && plot_sensors  &&  (T_count+C_count)>0 && CTD_count>0
    ...
    if T_count>0 && C_count>0
        ...
        sp(1) = subplot(1,2,1);
        h = plot(T_plot_data, P_data_fast, T_slow_data, P_data_slow);grid on
        set(h(1),'linewidth',2)
        xlabel('[  ^{\circ}C ]')
        legend(T_legend_string,  'location','SouthEast')

        sp(2) = subplot(1,2,2);
        h = plot(C_plot_data, P_data_fast, C_slow_data, P_data_slow);grid on
        set(h(1),'linewidth',2)
        xlabel('[ mS cm^{-1} ]')
    ...
    if ~exist('P_limits', 'var')
        P_limits = get(sp(1),'ylim');
    end
    set(sp,'ylim',P_limits)
    set(sp, 'ydir', 'rev')
    ylabel(sp(1),'\it P \rm [dBar]')
...
if  make_figures  && plot_sensors && ~ isempty(var_list)
    ...
    % - plot
    h = plot(Y_fast, P_data_fast, Y_slow, P_data_slow);grid on
...
%     set(gcf,'Position',[100 100 1000 500])
    ...
    for ii = 1:2
        ...
        for kk = 1:length(plot_gradients)
            ...
            elseif strcmp(plot_gradients{kk},'GRADT_LP')
                legend_list = strcat(gradT_legend_list,{' '},plot_gradients{kk}(end-1:end));
        ...
        % - plot
        plot(X_gradient_data, P_fast ); grid on
    ...
    % - plot T
    plot(sp(3),T_data,P_fast,'Parent',sp(3));
    
    % - limits and labels
    set(sp(3),'YDir','rev','Ylim',P_limits)
    ...
    if ~isempty(C_data)
        ...
        plot(sp(4),C_data,P_fast,'color',col);
        set(sp(4),'XAxisLocation','top',...
            'YAxisLocation','right','Color','none',...
            'XColor',col,'YColor','none',...
            'YDir','rev','Ylim',P_limits)
        % - limits
        x_lim = get(sp(4),'Xlim');
        if diff(x_lim)<0.1
...
if ~isempty(P_start) && ~isempty(P_end)
    range = find((P_fast > P_start) & (P_fast <= P_end));
    if isempty(range)
...
if ~isempty(range) && make_figures && plot_spectra
    ...
    new_title_string = [title_string , ...
        sprintf('%d < P < %d m, speed = %0.2f m s^{-1}, f_{HP} = %0.2f Hz', ...
        min(P_start,P_end), max(P_start,P_end), mean_speed, HP_cut)];
    ...
    new_title_string{size(new_title_string,2)+1} = ...
        [ num2str(min(P_start,P_end)) ' < P < ' num2str(max(P_start,P_end)) ' m, f_{HP} = ' num2str(HP_cut) ' Hz'];
    title(new_title_string)
    drawnow
end
